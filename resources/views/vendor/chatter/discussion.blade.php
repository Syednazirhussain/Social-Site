...
<div id="chatter" class="discussion">
	...
	<div id="chatter_header" style="background-color:{{ $discussion->color }}">
		<div class="container">
			<a class="back_btn" href="{{ url('/'.Config::get('chatter.routes.home')) }}"><i class="chatter-back"></i></a>
			<h1>{{ $discussion->title }}</h1><span class="chatter_head_details"> @lang('chatter::messages.discussion.head_details')<a class="chatter_cat" href="{{ url('/'.Config::get('chatter.routes.home')) }}/{{ Config::get('chatter.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
		</div>
	</div>
            ...
            @endif
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
			            ...
			            <div id="new_discussion">
				            ...
				            <form id="chatter_form_editor" action="{{ url('/'.Config::get('chatter.routes.home')) }}/posts" method="POST">
				...
				@else
					...
					<div id="login_or_register">
						<p>
                            @lang('chatter::messages.auth', ['home' => url('/'.Config::get('chatter.routes.home')) ])
                        </p>
					</div>
    ...
    @if(Config::get('chatter.sidebar_in_discussion_view'))
        <div id="new_discussion_in_discussion_view">
            ...
            <form id="chatter_form_editor_in_discussion_view" action="{{ url('/'.Config::get('chatter.routes.home')) }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
                <div class="row">
                    <div class="col-md-7">
                        <!-- TITLE -->
...
<script>
	$('document').ready(function(){
		...
		$('.delete_response').click(function(){
			...
			$.form("{{ url('/'.Config::get('chatter.routes.home')) }}/posts/" + post_id, { _token: '{{ csrf_token() }}', _method: 'DELETE'}, 'POST').submit();
		});
